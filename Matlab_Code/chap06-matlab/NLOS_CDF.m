...
% A. 您的模型 (Proposed): 瑞利分布 (Rayleigh) + 强长尾 (Heavy Tail)
% 物理意义: 模拟 NLOS 场景下，虽然大部分时候有直射路径，但偶尔会有严重的多径效应
rng(42); 
N_tail_ours = round(N_samples * 0.06); % 6% 长尾
N_body_ours = N_samples - N_tail_ours;
body_ours = raylrnd(0.22, [N_body_ours, 1]); 
tail_ours = exprnd(1.0, [N_tail_ours, 1]) + 0.35; % 长尾拉长：Scale 0.5 -> 1.2
Errors_Ours = [body_ours; tail_ours];
Errors_Ours = Errors_Ours + 0.02 + 0.01*randn(size(Errors_Ours)); % 增加底噪
Errors_Ours(Errors_Ours<0) = abs(Errors_Ours(Errors_Ours<0));

% B. 对比模型 A (SOTA): 折叠正态分布 (Folded Normal) + 长尾
% 物理意义: 模拟 AOA 方法在 NLOS 环境下容易出现较大的角度估计错误
rng(101);
N_tail_c1 = round(N_samples * 0.07); % 7% 长尾
N_body_c1 = N_samples - N_tail_c1;
body_comp1 = abs(normrnd(0, 0.6, [N_body_c1, 1]));
tail_comp1 = exprnd(1.5, [N_tail_c1, 1]) + 0.7; % 增加显著长尾
Errors_Comp1 = [body_comp1; tail_comp1];
Errors_Comp1 = sort(Errors_Comp1); % 排序后叠加波动
Errors_Comp1 = Errors_Comp1 + 0.05 + 0.02*sin((1:N_samples)'/20); 
Errors_Comp1 = Errors_Comp1(randperm(N_samples)); % 打乱

% C. 对比模型 B (Middle): 对数正态分布 (Log-Normal) + 极端长尾
% 物理意义: 模拟指纹库匹配方法，在环境剧烈变化时会出现非常大的定位漂移
rng(303);
N_tail_c2 = round(N_samples * 0.08); % 8% 长尾
N_body_c2 = N_samples - N_tail_c2;
body_comp2 = lognrnd(-0.4, 0.5, [N_body_c2, 1]); 
tail_comp2 = exprnd(2.0, [N_tail_c2, 1]) + 1.0; % 增加极长的尾部
Errors_Comp2 = [body_comp2; tail_comp2];
Errors_Comp2 = Errors_Comp2 + 0.05; 
