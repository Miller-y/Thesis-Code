...
class CSIDataset(Dataset):
    def __init__(self, mat_file_path):
        ...
        csi_data = mat_data['data'] # 预期形状: (3000, 8, 108, 100) 或类似
        ...
        # 维度调整逻辑
        # 目标 PyTorch 输入: (N, C, H, W) = (样本数, 8, 108, 100)
        # C=8 (天线/设备), H=108 (子载波/频率), W=100 (时间/帧)
        
        # 检查输入维度并自动调整 (简单的启发式规则)
        if csi_data.shape[1] == 8 and csi_data.shape[2] == 108 and csi_data.shape[3] == 100:
            # 已经是 (N, 8, 108, 100)
            ...
            # H 是 108
            # W 是 100
            # 这里根据您提供的描述 "3000x8x108x100"，假设加载后的 shape 就是这样。
            # 如果 MATLAB 保存时的顺序不同，这里可能需要 transpose。
            # 例如，如果 MATLAB 是 (100, 108, 8, 3000)，则需要 transpose(3, 2, 1, 0)
            
            # 暂时按您描述的 (3000, 8, 108, 100) 处理，不做额外 transpose，除非维度不匹配
            self.csi_data = csi_data
        else:
            raise ValueError(f"CSI数据维度不符合预期: {csi_data.shape}")

        # 预处理：去噪 (6阶巴特沃斯低通滤波器)
        # 场景: 室内定位 (慢速移动 0.1m/s) -> Doppler频移约 1.6Hz
        # 采样率: 100Hz
        # 截止频率建议: 5Hz (保留运动特征，滤除高频抖动)
        # 沿时间轴 (最后一维 W=100) 滤波
        self.csi_data = self.apply_filter(self.csi_data, cutoff=5, fs=100) # 修正fs为100, cutoff为5
        ...
        # 全局归一化或按特征归一化
        # 为了保持时频结构，建议将所有 CSI 值视为同一分布进行归一化，或者按子载波归一化
        # 这里修改为：将 (N, 8, 108, 100) 展平为 (N, -1) 进行 Sample-wise 归一化，或者全局归一化
        # 原代码逻辑是 reshape(-1, shape[-1]) 即按时间步归一化，这可能会破坏时间特征
        # 修改为：全局归一化 (所有数值缩放到 0-1)
        N, C, H, W = self.csi_data.shape
        csi_data_flat = self.csi_data.reshape(-1, 1)
        self.csi_data = torch.tensor(
            self.csi_scaler.fit_transform(csi_data_flat).reshape(N, C, H, W),
            dtype=torch.float32
        )
...
class MSTC(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 并行四个卷积分支，核大小分别为 1×3、1×9、1×15、1×25
        branch_channels = out_channels // 4
        self.branch1 = nn.Conv2d(in_channels, branch_channels, kernel_size=(1, 3), padding=(0, 1))
        self.branch2 = nn.Conv2d(in_channels, branch_channels, kernel_size=(1, 9), padding=(0, 4))
        self.branch3 = nn.Conv2d(in_channels, branch_channels, kernel_size=(1, 15), padding=(0, 7))
        self.branch4 = nn.Conv2d(in_channels, branch_channels, kernel_size=(1, 25), padding=(0, 12))
...
def train_model(model, train_loader, test_loader, criterion, optimizer, scheduler, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        scheduler.step(loss_history['train'][-1])
...
if __name__ == "__main__":
    ...
    train_dataset = CSIDataset('D:/lihan/室内定位/实验/all/打乱/十字点-中间点-打乱-幅值.mat')
    test_dataset = CSIDataset('D:/lihan/室内定位/实验/all/轨迹-幅值1.mat')

    # 注意：根据您的描述（0.1m/s 慢速连续行走，3000个连续采样点），
    # 数据具有极强的时空连续性。
    # 1. 必须设置 shuffle=False，以保证 Batch 内的数据顺序与真实轨迹一致，
    #    从而使 CustomLoss 中的平滑损失 (Ls) 能够正确计算相邻点的速度差。
    # 2. 由于移动速度较慢 (0.1m/s)，相邻点的位置变化很小，Ls 能有效抑制预测值的随机跳变（Jitter）。
    ...
    # 使用自定义混合损失函数
    # 注意：为了使平滑损失(Ls)有效，训练数据最好是连续的轨迹片段。
    # 如果数据是完全打乱的，Ls 的效果会打折扣（变为约束任意两点间的相对误差）。
    criterion = CustomLoss(beta=1.0, eta1=1.0, eta2=0.5)
